...
class TypeConverterDelegate {
     ...
     */
    @Nullable
    <T> T convertIfNecessary(@Nullable String propertyName, @Nullable Object oldValue,
                             Object newValue, @Nullable Class<T> requiredType) throws IllegalArgumentException {
        return convertIfNecessary(propertyName, oldValue, newValue, requiredType,
    ...
    <T> T convertIfNecessary(@Nullable String propertyName, @Nullable Object oldValue,
        ...
        PropertyEditor editor = this.propertyEditorRegistry.findCustomEditor(requiredType,
                propertyName);
        ...
        if (editor != null || (requiredType != null && !ClassUtils.isAssignableValue(requiredType
                , convertedValue))) {
            if (typeDescriptor != null && requiredType != null && Collection.class.isAssignableFrom(requiredType) && convertedValue instanceof String) {
                ...
                if (elementTypeDesc != null) {
                    ...
                    if (Class.class == elementType || Enum.class.isAssignableFrom(elementType)) {
                        convertedValue =
                                StringUtils.commaDelimitedListToStringArray((String) convertedValue);
                    }
                }
            }
            if (editor == null) {
        ...
        if (requiredType != null) {
            ...
            if (convertedValue != null) {
                if (Object.class == requiredType) {
                    ...
                    return (T) convertToTypeArray(convertedValue, propertyName,
                    ...
                    convertedValue = convertToTypedMap((Map<?, ?>) convertedValue, propertyName,
                ...
                if (String.class == requiredType && ClassUtils.isPrimitiveOrWrapper(convertedValue.getClass())) {
                    ...
                    convertedValue = attemptToConvertStringToEnum(requiredType, trimmedValue,
                            convertedValue);
                    standardConversion = true;
                } else if (convertedValue instanceof Number && Number.class.isAssignableFrom(requiredType)) {
                    convertedValue =
                            NumberUtils.convertNumberToTargetClass((Number) convertedValue,
    ...
    private Object attemptToConvertStringToEnum(Class<?> requiredType, String trimmedValue,
        ...
        if (Enum.class == requiredType && this.targetObject != null) {
            ...
            if (index > -1) {
                ...
                try {
                    Class<?> enumTypeValue = ClassUtils.forName(enumType, cl);
                    Field enumField = enumTypeValue.getField(fieldName);
    ...
    private Object doConvertValue(@Nullable Object oldValue, @Nullable Object newValue,
        ...
        if (editor != null && !(convertedValue instanceof String)) {
            ...
            try {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("PropertyEditor [" + editor.getClass().getName() + "] does not " +
                            "support setValue call", ex);
                }
                // Swallow and proceed.
            }
        }
        ...
        if (convertedValue instanceof String) {
            if (editor != null) {
                ...
                if (logger.isTraceEnabled()) {
                    logger.trace("Converting String to [" + requiredType + "] using property " +
                            "editor [" + editor + "]");
                }
     ...
     */
    private Object doConvertTextValue(@Nullable Object oldValue, String newTextValue,
    ...
    private Object convertToTypeArray(Object input, @Nullable String propertyName,
                                      Class<?> componentType) {
        if (input instanceof Collection) {
            ...
            Object value = convertIfNecessary(buildIndexedPropertyName(propertyName, 0), null,
                    input, componentType);
    ...
    private Collection<?> convertToTypedCollection(Collection<?> original,
        ...
        if (!approximable && !canCreateCopy(requiredType)) {
            if (logger.isDebugEnabled()) {
                logger.debug("Custom Collection type [" + original.getClass().getName() + "] does" +
                        " not allow for creating a copy - injecting original Collection as-is");
            }
            return original;
        }

        boolean originalAllowed = requiredType.isInstance(original);
        TypeDescriptor elementType = typeDescriptor != null ?
        ...
        try {
            if (approximable) {
                convertedCopy = CollectionFactory.createApproximateCollection(original,
                        original.size());
            } else {
                convertedCopy =
        ...
        for (; it.hasNext(); i++) {
            ...
            try {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("Collection type [" + original.getClass().getName() + "] seems " +
                            "to be read-only - injecting original Collection as-is: " + ex);
                }
                return original;
            }
    ...
    @SuppressWarnings("unchecked")
    private Map<?, ?> convertToTypedMap(Map<?, ?> original, @Nullable String propertyName, Class<
            ?> requiredType, @Nullable TypeDescriptor typeDescriptor) {
        ...
        if (!approximable && !canCreateCopy(requiredType)) {
            if (logger.isDebugEnabled()) {
                logger.debug("Custom Map type [" + original.getClass().getName() + "] does not " +
                        "allow for creating a copy - injecting original Map as-is");
            }
            return original;
        }

        boolean originalAllowed = requiredType.isInstance(original);
        TypeDescriptor keyType = typeDescriptor != null ?
                typeDescriptor.getMapKeyTypeDescriptor() : null;
        TypeDescriptor valueType = typeDescriptor != null ?
                typeDescriptor.getMapValueTypeDescriptor() : null;
        ...
        while (it.hasNext()) {
            ...
            try {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("Map type [" + original.getClass().getName() + "] seems to be " +
                            "read-only - injecting original Map as-is: " + ex);
                }
                return original;
            }
    ...
    private String buildIndexedPropertyName(@Nullable String propertyName, int index) {
        return propertyName != null ?
                propertyName + PropertyAccessor.PROPERTY_KEY_PREFIX + index + PropertyAccessor.PROPERTY_KEY_SUFFIX : null;
    }
