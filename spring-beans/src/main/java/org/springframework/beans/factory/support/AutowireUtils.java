...
abstract class AutowireUtils {
     ...
     */
    static boolean isExcludeFromDependencyCheck(PropertyDescriptor pd) {
    ...
    static boolean isSetterDefinedInInterface(PropertyDescriptor pd, Set<Class<?>> interfaces) {
        ...
        if (setter != null) {
            ...
            for (Class<?> ifc : interfaces) {
                if (ifc.isAssignableFrom(targetClass) && ClassUtils.hasMethod(ifc,setter.getName(),
            ...
            for (int i = 0;i < methodParameterTypes.length;i ++) {
                ...
                if (methodParameterType.equals(genericReturnType)) {
                    if (arg instanceof TypedStringValue) {
                        ...
                        try {
                            Class<?> reslovedType = typedValue.resolveTargetType(classLoader);
                            if (reslovedType != null) {
                                return reslovedType;
                            }
